...
- (BOOL)resetDb {
    ...
    [self.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        if (hadError)
            NSLog(@"[SUSSubFolderLoader] Err %d: %@", [db lastErrorCode], [db lastErrorMessage]);
    }];
...
- (BOOL)insertSongIntoFolderCache:(ISMSSong *)aSong {
    __block BOOL hadError;
    [self.dbQueue inDatabase:^(FMDatabase *db) {
        [db executeUpdate:[NSString stringWithFormat:@"INSERT INTO songsCache (folderId, %@) VALUES (?, %@)", [ISMSSong standardSongColumnNames], [ISMSSong standardSongColumnQMarks]], self.myId.md5, aSong.title, aSong.songId, aSong.artist, aSong.album, aSong.genre, aSong.coverArtId, aSong.path, aSong.suffix, aSong.transcodedSuffix, aSong.comment, aSong.duration, aSong.bitRate, aSong.track, aSong.year, aSong.size, aSong.parentId, NSStringFromBOOL(aSong.isVideo), aSong.discNumber];
        
        NSLog(@"[SUSSubFolderLoader] Added to folderCache with discNumber: %@", aSong.discNumber);
        
        hadError = [db hadError];
        if (hadError)
            NSLog(@"[SUSSubFolderLoader] Err inserting song %d: %@", [db lastErrorCode], [db lastErrorMessage]);
    }];
...
- (BOOL)insertAlbumsCount {
    ...
    [self.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        hadError = [db hadError];
        if ([db hadError])
            NSLog(@"[SUSSubFolderLoader] Err inserting album count %d: %@", [db lastErrorCode], [db lastErrorMessage]);
    }];
...
- (BOOL)insertSongsCount {
    ...
    [self.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        hadError = [db hadError];
        if (hadError)
            NSLog(@"[SUSSubFolderLoader] Err inserting song count %d: %@", [db lastErrorCode], [db lastErrorMessage]);
    }];
...
- (BOOL)insertFolderLength {
    ...
    [self.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        hadError = [db hadError];
        if ([db hadError])
            NSLog(@"[SUSSubFolderLoader] Err inserting folder length %d: %@", [db lastErrorCode], [db lastErrorMessage]);
    }];
