...
- (void)startDownloadQueue
{
    ...
    NSLog(@"[ISMSCacheQueueManager] starting download queue for: %@", self.currentQueuedSong);
	...
	if (self.currentQueuedSong.isFullyCached) {
        NSLog(@"[ISMSCacheQueueManager] Marking %@ as downloaded because it's already fully cached", self.currentQueuedSong.title);
	...
	if (handler) {
        NSLog(@"[ISMSCacheQueueManager] stealing %@ from stream manager", handler.mySong.title);
        ...
        NSLog(@"[ISMSCacheQueueManager] CQ creating download handler for %@", self.currentQueuedSong.title);
...
- (void)ISMSStreamHandlerConnectionFinished:(ISMSStreamHandler *)handler {
    ...
    NSLog(@"[ISMSCacheQueueManager] finished download took %f seconds", [[NSDate date] timeIntervalSinceDate:start]);
}
